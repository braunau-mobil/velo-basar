    ...
    : AbstractValidator<CartModel>
{
    ...
    private async Task ValidateProductId(int id, ValidationContext<CartModel> context, CancellationToken cancellation)
    {
        ...
        {
            TransactionEntity transaction = await _transactionService.GetLatestForProductAsync(product.Id);
        ...
        {
            TransactionEntity sale = await _transactionService.GetLatestForProductAsync(product.Id);
