...
public class CreateSampleSaleDocumentAsync
    : TestBase
{
    ...
    [Theory]
    [VeloAutoData]
    public async void ShouldCreateRandomSaleAndDocument(SellerEntity seller, BasarEntity basar, ProductEntity product, byte[] documentBytes)
    {
        //  Arrange
        ...
        A.CallTo(() => DocumentService.CreateTransactionDocumentAsync(A<TransactionEntity>.That.Matches(transaction => transaction.Type == TransactionType.Sale))).Returns(documentBytes);

        //  Act
        FileDataEntity result = await Sut.CreateSampleSaleDocumentAsync();
        ...
        A.CallTo(() => DocumentService.CreateTransactionDocumentAsync(A<TransactionEntity>.That.Matches(transaction => transaction.Type == TransactionType.Sale))).MustHaveHappenedOnceExactly();
    }
}
