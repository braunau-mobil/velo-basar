    ...
    : TestBase<EmptySqliteDbFixture>
{
    [Theory]
    [AutoData]
    public async Task OneProduct_ShouldRemoveProductFromSaleAndCreateCancellation(BasarEntity basar, DateTime timestamp, int number, byte[] document)
    {
        // Arrange
        ...
        NumberService.Setup(_ => _.NextNumberAsync(basar.Id, TransactionType.Cancellation))
            .ReturnsAsync(number);
        TransactionDocumentService.Setup(_ => _.CreateAsync(It.IsAny<TransactionEntity>()))
        ...
        TransactionDocumentService.Verify(_ => _.CreateAsync(It.IsAny<TransactionEntity>()), Times.Once());
        VerifyNoOtherCalls();
    }
}
