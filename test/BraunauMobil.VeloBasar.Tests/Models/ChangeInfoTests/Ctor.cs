...
namespace BraunauMobil.VeloBasar.Tests.Models.ChangeInfoTests
{
    public class Ctor
    {
        ...
        [InlineData(87.99, 
            ...
            1, 2, 0, 
            1, 2, 0)]
        public void OneProduct(decimal amount, int count500, int count200, int count100, int count50, int count20, int count10, int count5, int count2, int count1, int count050, int count020, int count010, int count005, int count002, int count001)
        {
            var settings = new VeloSettings();

            var changeInfo = new ChangeInfo(amount, settings.Nominations);
